...
class Min_heap:
    ...
    discovered edge. It creates and initialises the array with (None, None) since heap needs to start an array from
    ...
    """
    Method smallest_child determines the smallest child of current edge(based on input of k) and return its postion.
    Big-O Complexity: O(1)
    """
...
def dijkstra(n, roads, start, end):
    ...
    my_graph.vertices[start].heap_id = 1
    ...
    while len(discovered.array) > 1:
        ...
        for edge in my_graph.vertices[serving_vertex.id].edges:
            ...
            # if the city has not been discovered yet, update the vertex(u) details
            if not v.discovered:
                v.discovered = True
...
def opt_delivery(n, roads, start, end, delivery):
    ...
    if normal_optimal <= pick_up_optimal:
        ...
        # merge all the three routes together. O(R), T(3R)
